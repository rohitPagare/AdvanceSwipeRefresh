package com.orhotech.advanceswiperefreshlayout;
...
import static com.orhotech.advanceswiperefreshlayout.util.SmartUtil.dp2px;
import static com.orhotech.advanceswiperefreshlayout.util.SmartUtil.fling;
...
import com.orhotech.advanceswiperefreshlayout.api.RefreshComponent;
...
import com.orhotech.advanceswiperefreshlayout.constant.DimensionStatus;
...
import com.orhotech.advanceswiperefreshlayout.listener.DefaultRefreshFooterCreator;
import com.orhotech.advanceswiperefreshlayout.listener.DefaultRefreshHeaderCreator;
import com.orhotech.advanceswiperefreshlayout.listener.DefaultRefreshInitializer;
...
import com.orhotech.advanceswiperefreshlayout.wrapper.RefreshContentWrapper;
import com.orhotech.advanceswiperefreshlayout.wrapper.RefreshFooterWrapper;
import com.orhotech.advanceswiperefreshlayout.wrapper.RefreshHeaderWrapper;
...
public class AdvanceSwipeRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {

    //<editor-fold desc="property variable property and variable">

    //<editor-fold desc="sliding attribute">
    protected int mTouchSlop;
    protected int mSpinner; // Current Spinner: a value greater than 0 indicates pull-down, and a value less than 0 indicates pull-up
    protected int mLastSpinner; // Last Spinner
    protected int mTouchSpinner; // Spinner when touched
    protected int mFloorDuration = 300; // Second floor expansion duration
    protected int mReboundDuration = 300; // Rebound animation duration
    protected int mScreenHeightPixels; // Screen height
    protected float mTouchX;
    protected float mTouchY;
    protected float mLastTouchX; // Used to achieve the left and right drag effect of Header
    protected float mLastTouchY; // Used to implement multi-touch
    protected float mDragRate = .5f;
    protected char mDragDirection = 'n'; // Drag direction none-n horizontal-h vertical-v
    protected boolean mIsBeingDragged; // Whether it is being dragged
    ...
    protected int mFixedHeaderViewId = View.NO_ID; // View Id fixed at the top
    protected int mFixedFooterViewId = View.NO_ID; // View Id fixed at the bottom
    protected int mHeaderTranslationViewId = View.NO_ID; // View Id with Header offset
    protected int mFooterTranslationViewId = View.NO_ID; // View Id with Footer offset
    ...
    //<editor-fold desc="Functional attributes">
    protected int[] mPrimaryColors;
    protected boolean mEnableRefresh = true;
    protected boolean mEnableLoadMore = false;
    protected boolean mEnableClipHeaderWhenFixedBehind = true; // Whether to clip and block Header when Header is FixedBehind
    protected boolean mEnableClipFooterWhenFixedBehind = true; // Whether to clip and block Footer when Footer is FixedBehind
    protected boolean mEnableHeaderTranslationContent = true; // Whether to enable content view drag effect
    protected boolean mEnableFooterTranslationContent = true; // Whether to enable content view drag effect
    protected boolean mEnableFooterFollowWhenNoMoreData = false; // Whether Footer follows the content after all loading is completed 1.0.4-6
    protected boolean mEnablePreviewInEditMode = true; // Whether to enable preview function in edit mode
    protected boolean mEnableOverScrollBounce = true; // Whether to enable overscroll bounce
    protected boolean mEnableOverScrollDrag = false; // Whether to enable over-boundary drag (imitating Apple effect) 1.0.4-6
    ...
    protected boolean mEnableScrollContentWhenLoaded = true; // Whether to scroll the content to display new data after loading more is completed
    protected boolean mEnableScrollContentWhenRefreshed = true; // Whether to scroll the content to display new data after refresh is completed
    protected boolean mEnableLoadMoreWhenContentNotFull = true; // Whether you can pull up to load more when the content is not full page
    protected boolean mEnableNestedScrolling = true; // Whether to enable nested scrolling
    protected boolean mDisableContentWhenRefresh = false; // Whether to enable disabling the content view when refreshing
    protected boolean mDisableContentWhenLoading = false; // Whether to enable disabling the content view when refreshing
    protected boolean mFooterNoMoreData = false; // Whether the data is fully loaded, if it is completed, it cannot trigger the loading event
    protected boolean mFooterNoMoreDataEffective = false; // Whether NoMoreData takes effect (some Footer may not support)

    protected boolean mManualLoadMore = false; // Whether LoadMore has been set manually, used for intelligent startup
    protected boolean mManualHeaderTranslationContent = false; // Whether the content view drag effect has been set manually
    protected boolean mManualFooterTranslationContent = false; // Whether the content view drag effect has been set manually
    //</editor-fold>

    //<editor-fold desc="Listener attributes">
    ...
    protected float mHeaderMaxDragRate = 2.5f; // Maximum drag ratio (maximum height / Header height)
    protected float mFooterMaxDragRate = 2.5f; // Maximum drag ratio (maximum height / Footer height)
    protected float mHeaderTriggerRate = 1.0f; // Trigger refresh distance to HeaderHeight ratio
    protected float mFooterTriggerRate = 1.0f; // Trigger loading distance to FooterHeight ratio

    protected float mTwoLevelBottomPullUpToCloseRate = 1 / 6f; // The ratio of the area occupied by the second-level refresh when it is opened and then swiped up at the bottom to close

    protected RefreshComponent mRefreshHeader; // Drop-down header view
    protected RefreshComponent mRefreshFooter; // Pull-up bottom view
    protected RefreshContent mRefreshContent; // Display content view
    //</editor-fold>
    ...
    /**
     * [Main state]
     * Scroll status facing SmartRefresh external
     */
    protected RefreshState mState = RefreshState.None; // Main state
    /**
     * [Additional status]
     * Scroll status when the main state mState is Refreshing or Loading
     * 1. When mState=Refreshing|Loading, mViceState may be different from mState
     * 2. When mState=None and over-boundary drag is enabled, mViceState may be different from mState
     * 3. In other states, it is equal to the main state mViceState=mState
     * 4. SmartRefresh cannot detect mViceState externally
     */
    protected RefreshState mViceState = RefreshState.None; // Sub-state (scroll state when the main state is refreshed)

    protected long mLastOpenTime = 0; // Last refresh or load time

    protected int mHeaderBackgroundColor = 0; // Draw a solid color background for Header
    protected int mFooterBackgroundColor = 0;

    protected boolean mHeaderNeedTouchEventWhenRefreshing; // Provide independent events for the game Header
    protected boolean mFooterNeedTouchEventWhenLoading;

    protected boolean mAttachedToWindow; // Whether to add to Window

    protected boolean mFooterLocked = false; // Whether the list cannot scroll up when Footer is loading

    /*
     * https://github.com/scwang90/SmartRefreshLayout/issues/1540
     * Problem repair auxiliary variable, record the trigger time and coordinates of the gesture event ACTION_DOWN when the close animation is executed
     */
    protected long mLastTimeOnActionDown = 0;
    protected float mLastTouchXOnActionDown = 0;
    protected float mLastTouchYOnActionDown = 0;

    // Global default Footer constructor
    protected static DefaultRefreshFooterCreator sFooterCreator = null;
    // Global default Header constructor
    protected static DefaultRefreshHeaderCreator sHeaderCreator = null;
    // Global default control initializer
    protected static DefaultRefreshInitializer sRefreshInitializer = null;
    // Default global layout Margin
...
//<editor-fold desc="Construction Methods">
    ...
    public AdvanceSwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AdvanceSwipeRefreshLayout);

        /*
         * After inheriting ViewGroup, AdvanceSwipeRefreshLayout behaves as if android:clipToPadding=true
         * even if not set. Specifically adding android:clipToPadding allows the Java code to check
         * whether it was set. If not set, enforce setClipToPadding(false).
         * android:clipChildren works similarly.
         */
        if (!ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_android_clipToPadding)) {
            super.setClipToPadding(false);
        }
        if (!ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_android_clipChildren)) {
            super.setClipChildren(false);
        }

        if (sRefreshInitializer != null) {
            sRefreshInitializer.initialize(context, this); // Call global initializer
        }

        mDragRate = ta.getFloat(R.styleable.AdvanceSwipeRefreshLayout_srlDragRate, mDragRate);
        mHeaderMaxDragRate = ta.getFloat(R.styleable.AdvanceSwipeRefreshLayout_srlHeaderMaxDragRate, mHeaderMaxDragRate);
        mFooterMaxDragRate = ta.getFloat(R.styleable.AdvanceSwipeRefreshLayout_srlFooterMaxDragRate, mFooterMaxDragRate);
        mHeaderTriggerRate = ta.getFloat(R.styleable.AdvanceSwipeRefreshLayout_srlHeaderTriggerRate, mHeaderTriggerRate);
        mFooterTriggerRate = ta.getFloat(R.styleable.AdvanceSwipeRefreshLayout_srlFooterTriggerRate, mFooterTriggerRate);
        mEnableRefresh = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableRefresh, mEnableRefresh);
        mReboundDuration = ta.getInt(R.styleable.AdvanceSwipeRefreshLayout_srlReboundDuration, mReboundDuration);
        mEnableLoadMore = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableLoadMore, mEnableLoadMore);
        mHeaderHeight = ta.getDimensionPixelOffset(R.styleable.AdvanceSwipeRefreshLayout_srlHeaderHeight, mHeaderHeight);
        mFooterHeight = ta.getDimensionPixelOffset(R.styleable.AdvanceSwipeRefreshLayout_srlFooterHeight, mFooterHeight);
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.AdvanceSwipeRefreshLayout_srlHeaderInsetStart, mHeaderInsetStart);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.AdvanceSwipeRefreshLayout_srlFooterInsetStart, mFooterInsetStart);
        mDisableContentWhenRefresh = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlDisableContentWhenLoading, mDisableContentWhenLoading);
        mEnableHeaderTranslationContent = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableHeaderTranslationContent, mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableFooterTranslationContent, mEnableFooterTranslationContent);
        mEnablePreviewInEditMode = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnablePreviewInEditMode, mEnablePreviewInEditMode);
        mEnableAutoLoadMore = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableAutoLoadMore, mEnableAutoLoadMore);
        mEnableOverScrollBounce = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableOverScrollBounce, mEnableOverScrollBounce);
        mEnablePureScrollMode = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableScrollContentWhenLoaded, mEnableScrollContentWhenLoaded);
        mEnableScrollContentWhenRefreshed = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableScrollContentWhenRefreshed, mEnableScrollContentWhenRefreshed);
        mEnableLoadMoreWhenContentNotFull = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableLoadMoreWhenContentNotFull, mEnableLoadMoreWhenContentNotFull);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableFooterFollowWhenLoadFinished, mEnableFooterFollowWhenNoMoreData);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableFooterFollowWhenNoMoreData, mEnableFooterFollowWhenNoMoreData);
        mEnableClipHeaderWhenFixedBehind = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableClipHeaderWhenFixedBehind, mEnableClipHeaderWhenFixedBehind);
        mEnableClipFooterWhenFixedBehind = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableClipFooterWhenFixedBehind, mEnableClipFooterWhenFixedBehind);
        mEnableOverScrollDrag = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableOverScrollDrag, mEnableOverScrollDrag);
        mFixedHeaderViewId = ta.getResourceId(R.styleable.AdvanceSwipeRefreshLayout_srlFixedHeaderViewId, mFixedHeaderViewId);
        mFixedFooterViewId = ta.getResourceId(R.styleable.AdvanceSwipeRefreshLayout_srlFixedFooterViewId, mFixedFooterViewId);
        mHeaderTranslationViewId = ta.getResourceId(R.styleable.AdvanceSwipeRefreshLayout_srlHeaderTranslationViewId, mHeaderTranslationViewId);
        mFooterTranslationViewId = ta.getResourceId(R.styleable.AdvanceSwipeRefreshLayout_srlFooterTranslationViewId, mFooterTranslationViewId);
        mEnableNestedScrolling = ta.getBoolean(R.styleable.AdvanceSwipeRefreshLayout_srlEnableNestedScrolling, mEnableNestedScrolling);
        mNestedChild.setNestedScrollingEnabled(mEnableNestedScrolling);

        mManualLoadMore = mManualLoadMore || ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_srlEnableLoadMore);
        mManualHeaderTranslationContent = mManualHeaderTranslationContent || ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_srlEnableHeaderTranslationContent);
        mManualFooterTranslationContent = mManualFooterTranslationContent || ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_srlEnableFooterTranslationContent);
        mHeaderHeightStatus = ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_srlHeaderHeight) ? DimensionStatus.XmlLayoutUnNotify : mHeaderHeightStatus;
        mFooterHeightStatus = ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_srlFooterHeight) ? DimensionStatus.XmlLayoutUnNotify : mFooterHeightStatus;

        int accentColor = ta.getColor(R.styleable.AdvanceSwipeRefreshLayout_srlAccentColor, 0);
        int primaryColor = ta.getColor(R.styleable.AdvanceSwipeRefreshLayout_srlPrimaryColor, 0);
        if (primaryColor != 0) {
            if (accentColor != 0) {
...
//</editor-fold>
    ...
    /**
     * Rewrite onFinishInflate to complete smart specific functions
     * 1. Intelligently find Content, Header, Footer defined in Xml
     */
    @Override
        ...
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mRefreshListener != null) {
                    if (notify) {
                        mRefreshListener.onRefresh(AdvanceSwipeRefreshLayout.this);
                    }
                } else if (mOnMultiListener == null) {
                    finishRefresh(3000);
                }
                if (mRefreshHeader != null) {
                    ...
                    mRefreshHeader.onStartAnimator(AdvanceSwipeRefreshLayout.this, mHeaderHeight, maxDragHeight);
                }
                if (mOnMultiListener != null && mRefreshHeader instanceof RefreshHeader) {
                    if (notify) {
                        mOnMultiListener.onRefresh(AdvanceSwipeRefreshLayout.this);
                    }
                    final int maxDragHeight = (int) (mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
...
//            reboundAnimator.setDuration(20000);
    ...
    protected void overSpinner() {
        if (mState == RefreshState.TwoLevel) {
             ...
             */
            Log.d("AdvanceSwipeRefreshLayout", "overSpinner 时 LoadFinish 状态无任何操作即可");
        } else if (mSpinner != 0) {
            mKernel.animSpinner(0);
        }
    }
    ...
    protected void moveSpinnerInfinitely(float spinner) {
         ...
         *
         * 1.全局关闭（推荐）
         *         AdvanceSwipeRefreshLayout.setDefaultRefreshInitializer(new DefaultRefreshInitializer() {
         *             @Override
         ...
         *
         * 2.XML关闭
         *          <com.scwang.smart.refresh.layout.AdvanceSwipeRefreshLayout
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && spinner < 0
            ...
            mHandler.postDelayed(() -> {
                if (mLoadMoreListener != null) {
                    mLoadMoreListener.onLoadMore(AdvanceSwipeRefreshLayout.this);
                } else if (mOnMultiListener == null) {
                    finishLoadMore(2000);//如果没有任何加载监听器，两秒之后自动关闭
                }
                final OnLoadMoreListener listener = mOnMultiListener;
                if (listener != null) {
                    listener.onLoadMore(AdvanceSwipeRefreshLayout.this);
                }
            }, mReboundDuration);
        }
    }
...
//    }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AdvanceSwipeRefreshLayout_Layout);
            backgroundColor = ta.getColor(R.styleable.AdvanceSwipeRefreshLayout_Layout_layout_srlBackgroundColor, backgroundColor);
            if (ta.hasValue(R.styleable.AdvanceSwipeRefreshLayout_Layout_layout_srlSpinnerStyle)) {
                spinnerStyle = SpinnerStyle.values[ta.getInt(R.styleable.AdvanceSwipeRefreshLayout_Layout_layout_srlSpinnerStyle, SpinnerStyle.Translate.ordinal)];
            }
            ta.recycle();
        }
...
//                  mRefreshFooter.getView().requestLayout();
     ...
     *
     * @param enabled 是否启用
     * @return AdvanceSwipeRefreshLayout
     */
    @Override
...
//    }
     ...
     * Finish refreshing.
     *
     * @param delayed Delay before finishing refresh (in milliseconds).
     * @param success Indicates whether the data refresh was successful (affects last update time).
     * @return RefreshLayout
     */
    @Override
...
//                      mKernel.setState(RefreshState.None);
                        ...
                        if (noMoreData == Boolean.FALSE) {
                            setNoMoreData(false);//NoMoreData can be reset only when there is a real refresh state
                        }
                    }
                    ...
                    int startDelay = mRefreshHeader.onFinish(AdvanceSwipeRefreshLayout.this, success);
                    if (mOnMultiListener != null && mRefreshHeader instanceof RefreshHeader) {
                        mOnMultiListener.onHeaderFinish((RefreshHeader) mRefreshHeader, success);
                    }
                    //startDelay < Integer.MAX_VALUE means that after delaying startDelay milliseconds, the rebound will close and refresh
                    if (startDelay < Integer.MAX_VALUE) {
                        //If you are dragging, offset the initial click event [In both cases, when the refresh is finished, the finger is still holding the screen and does not let go]
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                mTouchSpinner = 0;
                                mIsBeingDragged = false;
                                AdvanceSwipeRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                                AdvanceSwipeRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                AdvanceSwipeRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                                mNestedInProgress = false;
                                mTouchSpinner = 0;
                            }
                        }
...
//                            resetStatus();
    ...
    /**
     * Finish refreshing and indicate that there is no more data.
     *
     * @return RefreshLayout
     */
    @Override
     ...
     * Finish loading more.
     *
     * @param success Whether data loading was successful
     * @return RefreshLayout
     */
    @Override
            ...
            public void run() {
                if (count == 0) {
                    if (mState == RefreshState.None && mViceState == RefreshState.Loading) {
                        //autoLoadMore is about to be executed but has not started
                        mViceState = RefreshState.None;
                    } else if (reboundAnimator != null && (mState.isDragging || mState == RefreshState.LoadReleased) && mState.isFooter) {
                        //autoLoadMore is executing but has not ended
                        /*
                         * 2020-3-15 BUG fixed
                         * https://github.com/scwang90/SmartRefreshLayout/issues/1019
                         * Fix the problem that autoRefresh cannot be closed occasionally because cancel triggers the end callback.
                         */
                        reboundAnimator.setDuration(0);//cancel will trigger the End call. You can judge 0 to determine whether it is canceled.
                        reboundAnimator.cancel();//Cancel and end calls will be triggered
                        reboundAnimator = null;
                        /*
                         * 2020-1-4 BUG repair
                         * https://github.com/scwang90/SmartRefreshLayout/issues/1104
                         * If the current state is PullDownToRefresh and mSpinner != 0
                         * mKernel.setState(RefreshState.None); internally calls animSpinner(0); to turn off the animation
                         * But PullDownToRefresh has the isDragging attribute, animSpinner(0); will not reset the None state
                         * Will cause PullDownToRefresh to persist, and overSpinner(); will start refreshing after clicking on the list
                         */
                        if (mKernel.animSpinner(0) == null) {
                            notifyStateChanged(RefreshState.None);
                        } else {
                        ...
                        count++;
                        mHandler.postDelayed(this, more);
                        //Set the state to LoadFinish in advance to prevent postDelayed from causing finishLoadMore to pass, and externally determine whether state is Loading.
                    ...
                    final int startDelay = mRefreshFooter.onFinish(AdvanceSwipeRefreshLayout.this, success);
                    if (mOnMultiListener != null && mRefreshFooter instanceof RefreshFooter) {
                        mOnMultiListener.onFooterFinish((RefreshFooter) mRefreshFooter, success);
                    }
                    if (startDelay < Integer.MAX_VALUE) {
                        //Calculate the offset by which the layout will move
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                AdvanceSwipeRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offsetY + mTouchSlop * 2, 0));
                                AdvanceSwipeRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offsetY, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                AdvanceSwipeRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                        ...
                        mHandler.postDelayed(() -> {
                            ...
                            if (mEnableScrollContentWhenLoaded && offset < 0) {
                                updateListener = mRefreshContent.scrollContentWhenFinished(mSpinner);
                                if (updateListener != null) {//If the content needs to scroll to display new data
                                    updateListener.onAnimationUpdate(ValueAnimator.ofInt(0, 0));//Scroll directly, distance of Footer
                                }
                            }
                            ValueAnimator animator = null;//Motion animation and animation end callback
                            AnimatorListenerAdapter listenerAdapter = new AnimatorListenerAdapter() {
                                @Override
                                public void onAnimationEnd(Animator animation) {
                                    if (animation != null && animation.getDuration() == 0) {
                                        return;//0 means canceled
                                    }
                                    mFooterLocked = false;
                                    if (noMoreData) {
                                        setNoMoreData(true);
                            ...
                            if (mSpinner > 0) { //Greater than 0 means drop-down. This is where the Header is visible and the Footer is invisible.
                                animator = mKernel.animSpinner(0);//Close Header and return to original state
                            } else if (updateListener != null || mSpinner == 0) {//If both Header and Footer are invisible or the content needs to scroll to display new content
                                if (reboundAnimator != null) {
                                    reboundAnimator.setDuration(0);//cancel will trigger the End call. You can judge 0 to determine whether it is canceled.
                                    reboundAnimator.cancel();//Cancel and end calls will be triggered
                                    reboundAnimator = null;//Cancel any previous animation
                                }
                                //Close the Header directly or return the Header to the original state
                                mKernel.moveSpinner(0, false);
                                mKernel.setState(RefreshState.None);
                            } else {//Prepare to close Footer according to normal logic
                                if (noMoreData && mEnableFooterFollowWhenNoMoreData) {//If you need to show that there is no more data
                                    if (mSpinner >= -mFooterHeight) {//If the Footer's position is within the visible range
                                        notifyStateChanged(RefreshState.None);//Notify the reset status directly without closing the Footer
                                    } else {//If the Footer's position exceeds the Footer display height (the probability of this situation should be very low, it will be triggered when the finger deliberately drags the Footer upward beyond the original position)
                                        animator = mKernel.animSpinner(-mFooterHeight);//Return the Footer to the full display position through animation
                                    }
                                } else {
                                    animator = mKernel.animSpinner(0);//Animation closes normally Footer
                                }
                            }
                            if (animator != null) {
                                animator.addListener(listenerAdapter);//If it is closed through animation, bind the animation end callback
                            } else {
                                listenerAdapter.onAnimationEnd(null);//If there is no animation, execute the end callback immediately (must be logical)
                            }
    ...
    public RefreshLayout closeHeaderOrFooter() {
        if (mState == RefreshState.None && (mViceState == RefreshState.Refreshing || mViceState == RefreshState.Loading)) {
            //autoRefresh autoLoadMore is about to be executed but has not started
            mViceState = RefreshState.None;
        }
        if (mState == RefreshState.Refreshing) {
            ...
            /*
             * closeHeaderOrFooter closing logic,
             * Help FalsifyHeader cancel refresh
             * State FalsifyFooter cancels loading
             */
            if (mKernel.animSpinner(0) == null) {
                notifyStateChanged(RefreshState.None);
            } else {
    ...
    public boolean autoRefresh(int delayed, final int duration, final float dragRate, final boolean animationOnly) {
        if (mState == RefreshState.None && isEnableRefreshOrLoadMore(mEnableRefresh)) {
            Runnable runnable = () -> {
                ...
                final View thisView = AdvanceSwipeRefreshLayout.this;
                mLastTouchX = thisView.getMeasuredWidth() / 2f;
                mKernel.setState(RefreshState.PullDownToRefresh);

                if (mRefreshHeader != null && mRefreshHeader.autoOpen(duration, dragRate, animationOnly)) {
                    /*
                     * Adding Header can implement autoOpen by yourself. Return true to indicate support, return False to indicate not supported, use the old version of autoOpen
                     */
                    return;
                }
    ...
    /**
     * Set the default Header builder
     *
     * @param creator Header builder
     */
    ...
    /**
     * Set the default Footer builder
     *
     * @param creator Footer builder
     */
    ...
    //<editor-fold desc="Discarded API">
    ...
    public class RefreshKernelImpl implements RefreshKernel {
        ...
        public RefreshLayout getRefreshLayout() {
            return AdvanceSwipeRefreshLayout.this;
        }
        ...
        //<editor-fold desc="state changes state changes">
        @Override
         ...
         * The name of moveSpinner comes from Google’s official { android.support.v4.widget.SwipeRefreshLayout#moveSpinner(float)}
         * moveSpinner The name comes from { android.support.v4.widget.SwipeRefreshLayout#moveSpinner(float)}
         *
         * @param spinner    new spinner
         * @param isDragging whether the scrolling is caused by dragging
         *                   Only the rebound animation of finishRefresh, finishLoadMore, and overSpinner will be false
         *                   dispatchTouchEvent, nestScroll, etc. are all true
         *                   autoRefresh, autoLoadMore, need to simulate drag, also true
         */
        public RefreshKernel moveSpinner(final int spinner, final boolean isDragging) {
            ...
            mSpinner = spinner;
            // The additional judgment of mViceState.isDragging is because isDragging is sometimes simulated by animation, such as autoRefresh animation.
            //
            ...
            if (mRefreshContent != null) {
                ...
                /*
                 * Fixed the problem that pure scrolling mode is invalid due to no default Header Footer after version 2.0
                 */
            ...
            if ((spinner >= 0 || oldSpinner > 0) && mRefreshHeader != null) {
                ...
                //Because the user may directly turn off enable=false after finish, so the state judgment of state must be added.
                if (isEnableRefreshOrLoadMore(mEnableRefresh) || (mState == RefreshState.RefreshFinish && !isDragging)) {
                    if (oldSpinner != mSpinner) {
                        if (mRefreshHeader.getSpinnerStyle() == SpinnerStyle.Translate) {
                            ...
                            /*
                             * Compatible with MotionLayout 2019-6-18
                             * requestLayout is invalid inside MotionLayout
                             * It is better to call the layout method directly
                             * https://github.com/scwang90/SmartRefreshLayout/issues/944
                             */
                            // mRefreshHeader.getView().requestLayout();
                            View headerView = mRefreshHeader.getView();
        ...
        public ValueAnimator animSpinner(int endSpinner) {
            return AdvanceSwipeRefreshLayout.this.animSpinner(endSpinner, 0, mReboundInterpolator, mReboundDuration);
        }
        //</editor-fold>
        ...
        public RefreshKernel onAutoRefreshAnimationEnd(Animator animation, boolean animationOnly) {
            ...
            if (mRefreshHeader != null) {
                ...
                AdvanceSwipeRefreshLayout.this.setStateRefreshing(!animationOnly);
            } else {
                /*
                 * Fixed the problem of status confusion when mRefreshHeader=null
                 */
        ...
        public RefreshKernel onAutoLoadMoreAnimationEnd(Animator animation, boolean animationOnly) {
            ...
            if (mRefreshFooter != null) {
                ...
                AdvanceSwipeRefreshLayout.this.setStateLoading(!animationOnly);
            } else {
                /*
                 * Fixed the problem of status confusion when mRefreshFooter=null
                 */
                this.setState(RefreshState.None);
            }
            return this;
